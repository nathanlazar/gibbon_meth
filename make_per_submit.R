...
write.lines(c('ID=$(Cluster).$(Process)',
  paste0('dir=', drive),
  'should_transfer_files = IF_NEEDED',
  'when_to_transfer_output = ON_EXIT',
  paste0('executable=', drive, executable),
  paste0('arguments=', paste(arguments)),
  ...
  paset0('request_memory=', req_mem),
  paste0('request_disk=' req_disk),
  paste0('queue', as.character(num_jobs))), 
file='condor.submit')
}
